...
let generateDiskOnBeamAxisForRadius
    (isocenter : VVector) // Center point of the plan/beam in DICOM coords
    (sourcePosition : VVector) // Radiation source position in DICOM coords
    (offset : float<mm>) // Distance from isocenter along axis toward the source (+) or away (−)
    (pointsPerDisk : int) // Number of points sampled on the disk perimeter
...
let generateDiskWithInterior
    ...
    |> List.map(fun r ->
        generateDiskOnBeamAxisForRadius isocenter sourcePosition offset pointsPerDisk r)
    |> List.concat
    


let generateDisksForBeamForRadius
    (beam : Beam) // Plan beam to sample
    (arcStep : float) // Step size in degrees for arc sampling; ignored for static beams
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerDisk : int) // Number of perimeter points per disk
    ...
    |> extractSourcePositions arcStep // (VVector * VVector)[] of (isocenter, source)
    |> Array.map (fun (iso, src) ->
        generateDiskWithInterior iso src offset pointsPerDisk radius radiusSteps
        |> List.toArray) // VVector[] for each disk
    |> Array.concat
    |> Array.toList
...
let generateSlicesForBeamForRadius
    (beam : Beam) // Plan beam to sample
    (arcStep : float) // Step size in degrees for arc sampling; ignored for static beams
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerLine : int) // Number of points sampled on each line
    (length : float<mm>) //Radius of the disk
    =
    beam
    |> extractSourcePositions arcStep // (VVector * VVector)[] of (isocenter, source)
    |> Array.map (fun (iso, src) ->
        generateLineOnBeamAxis iso src offset pointsPerLine length 
        |> List.toArray)
    |> Array.concat
    |> Array.toList

// Generate a list consisting of lines with disks at the end
let generateSlicesAndDisks
    (beam : Beam) // Plan beam to sample
    (arcStep : float) // Step size in degrees for arc sampling; ignored for static beams
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerDisk : int) // Number of perimeter points per disk
    ...
    //add disk points
    let diskPoints = 
        [|Array.head(beamPositions); Array.last(beamPositions)|]
        |> Array.map (fun (iso, src) ->
            generateDiskWithInterior iso src offset pointsPerDisk radius radiusSteps
            |> List.toArray)
...
let generateDisksForBeamModified
    (beamPositions: (VVector * VVector) array) // Plan beam to sample
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerDisk : int) // Number of perimeter points per disk
    =
    beamPositions
...
let generateDisksForBeamForRadiusModified
    (beamPositions: (VVector * VVector) array)
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerDisk : int) // Number of perimeter points per disk
    (radius : float<mm>) //Radius of the generated disks
    (radiusSteps : float<mm>) //Distance between each disk radii
...
let generateSlicesForBeamForRadiusModified
...
let generateSlicesAndDisksModified
    (srcPositions : (VVector*VVector) array)
    (offset : float<mm>) // Distance from isocenter along the beam axis for each disk
    (pointsPerDisk : int) // Number of perimeter points per disk
    (pointsPerLine : int)
    ...
    //add disk points
    let diskPoints = 
        [|Array.head(srcPositions); Array.last(srcPositions)|]
        |> Array.map (fun (iso, src) ->
            generateDiskWithInterior iso src offset pointsPerDisk radius radiusSteps
            |> List.toArray)
    Array.append diskPoints linePoints
...
let generateHalfDiskWithInteriorR
    ...
    let pointsPerDisk = 
        radii |> List.map(fun rad -> System.Math.PI*rad/resolution) |> List.map int
